...
由于每类员工（Person）的工资输入（input函数）、工资计算办法（pay函数,返回值类型为int）、显示信息（show函数）均不一样，因此使用多态来实现。
-----------------------------------------------------------------------------
请仔细阅读append.cc代码，并设计并实现这个员工类的派生体系，使main()函数能够运行并得到正确的输出。

Input
输入的第一个整数n，表示用n组测试样例。
每组测试样例占一行，分别为员工职位、员工姓名、基础工资base（int类型）、附加项k（Manager无此项）。
