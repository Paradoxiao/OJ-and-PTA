...
protected:
public:
  Character(char ch = 0) : ch(ch) {}
  Character operator+(int a) {
    return Character((ch - 'a' + a + 2600) % 26 + 'a');
  }
  Character operator-(int a) {
    return Character((ch - 'a' - a + 2600) % 26 + 'a');
  }
  friend istream &operator>>(istream &is, Character &other) {
    is >> other.ch;
    return is;
  }
  friend ostream &operator<<(ostream &os, const Character &other) {
    os << other.ch;
    return os;
  }
};
