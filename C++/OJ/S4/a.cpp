...
public:
  ...
  Data<T> operator+(const Data<T> &other) {
    return Data<T>(value + other.value);
  }
  friend ostream &operator<<(ostream &os, const Data<T> &other) {
    os << setprecision(2) << fixed << other.value;
    return os;
  }
};
template <typename T> class GetResult {
public:
  static T getSum(Data<T> *arr, int n) {
    T sum = 0;
    for (int i = 0; i < n; i++)
      sum += arr[i].getValue();
    return sum;
  }
  static T getMax(Data<T> *arr, int n) {
    int max = 0;
    for (int i = 1; i < n; i++)
      if (arr[i].getValue() > arr[max].getValue())
        max = i;
    return arr[max].getValue();
  }
  static T getMin(Data<T> *arr, int n) {
    int min = 0;
    for (int i = 1; i < n; i++)
      if (arr[i].getValue() < arr[min].getValue())
        min = i;
    return arr[min].getValue();
  }
};
int main() {
  Data<int> iData[1001];
